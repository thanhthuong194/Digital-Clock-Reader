...
def test_warp_logic(args):
    ...
    detector = ClockDetector(model_path=args.model, conf_threshold=0.4)
    ...
    if keypoints is not None:
        ...
        print("⚠️ No phone detected in this image.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Pose Detection + Perspective Transform")
    
    parser.add_argument('--image', type=str, required=True, help='Path to test image')
    parser.add_argument('--model', type=str, default='best.pt', help='Path to .pt model')
